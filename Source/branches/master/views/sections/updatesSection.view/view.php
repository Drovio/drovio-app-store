...
else
{
	...
	// Set update all action
	$updateAll = HTML::select(".marketUpdates .ubutton.update_all_button")->item(0);
	$actionFactory->setAction($updateAll, "sections/updates/updateAllApplications", "", $attr = array(), $loading = TRUE);
}
...
function getApplicationTile($teamApp, $actionFactory)
{
	...
	if (!empty($teamApp['lastVersion']) && $teamApp['lastVersion'] != $teamApp['version'])
	{
		...
		$updateBtn = DOM::create("div", $title, "", "ubutton update_button");
