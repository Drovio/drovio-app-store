...
if (engine::isPost())
{
	...
	// Update application
	$status = appMarket::buyApplication($applicationID, $applicationVersion);

	// If there is an error in updating the application, show it
	if ($status !== TRUE)
	{
		$err_header = appLiteral::get("purchases.dialog", "title");
		$err = $errFormNtf->addHeader($err_header);
		$errFormNtf->addDescription($err, DOM::create("span", "An error occurred while purchasing this application. Please try again later."));
...
$appContent->build("", "purchaseApplicationPermissionsDialog", TRUE);
...
{
	$appIcon = HTML::select(".purchaseApplicationPermissions .app_icon")->item(0);
	$img = DOM::create("img");
...
// title
$appTitle = HTML::select(".purchaseApplicationPermissions .hd.app_title")->item(0);
HTML::innerHTML($appTitle, $applicationInfo['projectTitle']);
// team
$teamName = HTML::select(".purchaseApplicationPermissions .hd.team_name")->item(0);
HTML::innerHTML($teamName, $applicationInfo['teamName']);
// version
$teamVersion = HTML::select(".purchaseApplicationPermissions .hd.version")->item(0);
$attr = array();
...
$pgiList = HTML::select(".purchaseApplicationPermissions .permissions .list")->item(0);
...
$noPermissions = HTML::select(".purchaseApplicationPermissions .hd.center")->item(0);
if (empty($manifests))
{
	// Clean the manifest list
	$mfContainer = HTML::select(".purchaseApplicationPermissions .permissions")->item(0);
...
// Build frame
$title = appLiteral::get("purchases.dialog", "title");
$df->build($title, $action = "", $background = TRUE)->engageApp("/sections/purchases/purchaseApplication");
